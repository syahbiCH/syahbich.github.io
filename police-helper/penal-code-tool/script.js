...
function populateChargesTable(filteredCharges = charges) {
    ...
    filteredCharges.forEach(charge => {
        ...
        row.innerHTML = `
            ...
            <td>${charge.fine}</td>
            <td>${charge.bail}</td>
            <td><button class="${buttonClass}" onclick="addCharge('${charge.number}', '${charge.description}', '${charge.fine}', '${charge.jailtime}', '${charge.bail}')">Add</button></td>
        `;
...
function addCharge(number, description, fine, jailtime, bail) {
    ...
    row.innerHTML = `
        ...
        <td>${bail}</td>
        <td>
            <button class="copy-btn" onclick="copyCharge('${number}', '${description}')">Copy</button>
...
function filterCharges() {
    ...
    if (searchTerm.includes(',')) {
        // Handle multiple charge numbers separated by commas
        const chargeNumbers = searchTerm.split(',').map(num => num.trim()).filter(num => num !== '');

        // Parse search terms into main categories and specific sub-charges
        const mainCategories = new Set();
        const subCharges = new Map();

        chargeNumbers.forEach(searchNum => {
            const normalized = searchNum.toLowerCase().replace(/[\(\)\s]/g, '');
            const match = normalized.match(/^(\d+)a(\d+)?$/i);
            if (match) {
                const main = match[1];
                const sub = match[2];
                if (sub) {
                    // Specific sub-charge
                    if (!subCharges.has(main)) {
                        subCharges.set(main, new Set());
                    }
                    subCharges.get(main).add(sub);
                } else {
                    // Main category
        ...
        // Filter charges based on parsed search terms
        filteredCharges = charges.filter(charge => {
            const codeNumber = getCodeNumber(charge.number);
            const normalizedCode = codeNumber.toLowerCase().replace(/[\(\)\s]/g, '');
            const match = normalizedCode.match(/^(\d+)a(\d+)?$/i);
            if (!match) return false;

            const chargeMain = match[1];
            const chargeSub = match[2];

            // Check if main category was searched
            if (mainCategories.has(chargeMain)) {
                return true; // Show all charges in this main category
            }

            // Check if specific sub-charge was searched
            if (subCharges.has(chargeMain) && chargeSub && subCharges.get(chargeMain).has(chargeSub)) {
                return true;
            }

            // Check for exact matches with other formats
            const displayNumber = getDisplayNumber(charge.number);
            const normalizedCharge = charge.number.toLowerCase();
            const normalizedDisplay = displayNumber.toLowerCase().replace(/[\(\)\s]/g, '');

            return chargeNumbers.some(searchNum => {
                const normalizedSearch = searchNum.toLowerCase().replace(/[\(\)\s]/g, '');
                return normalizedCharge.includes(normalizedSearch) || normalizedDisplay.includes(normalizedSearch) || normalizedCode.includes(normalizedSearch);
            });
        });
    } else {
        // Single search term (existing behavior)
        filteredCharges = charges.filter(charge => {
            const displayNumber = getDisplayNumber(charge.number);
            const codeNumber = getCodeNumber(charge.number);
...
function copyCharge(number) {
    const playerId = document.getElementById('playerId').value || '[PlayerID]';
    const command = `/su ${playerId} ${number}`;
...
function updateCodeAndCommand() {
    ...
    rows.forEach(row => {
        ...
        const jailtimeValue = cells[2].textContent.replace(' days', '');
        totalJailtime += jailtimeValue === '-' ? 0 : parseInt(jailtimeValue) || 0;
        totalFine += parseInt(cells[3].textContent.replace(/[$,]/g, '')) || 0;
        const bailValue = cells[4].textContent.replace(/[$,]/g, '');
...
function copyCommand() {
    ...
    navigator.clipboard.writeText(command).then(() => {
        alert('Command Arrest copied to clipboard: ' + command);
    });
}
