...
function populateChargesTable(filteredCharges = charges)
{
    ...
    filteredCharges.forEach(charge => {
        ...
        const buttonClass = isAdded ? 'added-btn' : 'search-action-btn';
...
function filterCharges() {
    ...
    else
    {
        ...
        filteredCharges = charges.filter(charge => {
            ...
            return searchTerms.some(term => codeNumber.includes(term) || desc.includes(term));
        });
    }
...
{
    const vehicleId = document.getElementById('vehicleId').value || '0';
    navigator.clipboard.writeText(`/markimpound ${vehicleId} ${number}. ${description}`);
}
